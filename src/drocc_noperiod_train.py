...
#Main work of Anomaly Detection
...
def load_data(path):
    train_data = ConnlogsDataset(path, "nn_oneclass", "train")
    validate_data = ConnlogsDataset(path, "nn_oneclass", "validate")
...
def run_test(nn, dataset_dir):
    ...
    test_dataset = ConnlogsDataset(dataset_dir, nn_type='nn_oneclass', tvt_type="test", timestamp=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mom', type=float, default=0.99, metavar='M',
                        help='momentum')
    parser.add_argument('--model_dir', default='../out/models/drocc_noperiod.pt',
    ...
    with open(os.path.join(args.result_path, "drocc_noperiod.csv"), 'w') as f_csv:
